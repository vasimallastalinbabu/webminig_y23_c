...
function GameCanvas() {
  ...
  useEffect(() => {
    ...
    mergeSoundRef.current = new Audio("/merge.wav");
    mergeSoundRef.current.volume = 0.55;

    bgmSoundRef.current = new Audio("/bgm.wav");
    bgmSoundRef.current.volume = 0.22;
  ...
  const playBgmSound = () => {
    ...
    bgmSoundRef.current.play().catch(() => {});
  };
  ...
  const playMergeSound = () => {
    ...
    if (!mergeSoundRef.current) return;

    const sound = mergeSoundRef.current.cloneNode();
    sound.volume = mergeSoundRef.current.volume;
    sound.play().catch(() => {});
  };
